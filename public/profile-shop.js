...
(()=>{
 ...
 const ACHIEVEMENTS=[
  ...
  {id:"streak3",icon:"🔥",name:"На волне",text:"Выиграть 3 партии подряд",reward:60},
  {id:"streak5",icon:"🔥",name:"Не остановить",text:"Выиграть 5 партий подряд",reward:120},
  {id:"streak10",icon:"☄️",name:"Легендарная серия",text:"Выиграть 10 партий подряд",reward:300},
  {id:"double_kill",icon:"⚔️",name:"Двойной удар",text:"Взять 2 фишки соперников за один ход",reward:50},
  {id:"comeback_win",icon:"🔄",name:"Камбэк",text:"Победить после отставания минимум на 2 фишки",reward:100},
  {id:"last_piece",icon:"🏁",name:"Осталась одна",text:"Завести четвёртую фишку в Домик",reward:25}
 ];
 const TABLE_VISUALS={
 ...
 function snapshot(){return {id:S.id,name:S.name,rating:S.rating,balance:S.balance,wins:S.wins,games:S.games,winStreak:S.winStreak,bestWinStreak:S.bestWinStreak,owned:S.owned,equipped:S.equipped,avatar:S.avatar,ownedTables:S.ownedTables,equippedTable:S.equippedTable,ownedDice:S.ownedDice,equippedDice:S.equippedDice,achievements:S.achievements}}
 function saveFallback(){try{localStorage.setItem("partis-profile",JSON.stringify(snapshot()))}catch{}}
 function loadFallback(){try{const x=JSON.parse(localStorage.getItem("partis-profile")||localStorage.getItem("mondavoshka-profile")||"null");if(!x)return;S.rating=+x.rating||0;S.balance=+x.balance||0;S.wins=+x.wins||0;S.games=+x.games||0;S.winStreak=+x.winStreak||0;S.bestWinStreak=+x.bestWinStreak||0;S.owned=Array.isArray(x.owned)?x.owned:["default"];S.equipped=x.equipped||"default";S.name=x.name||S.name;S.avatar=x.avatar||S.avatar;S.ownedTables=Array.isArray(x.ownedTables)?x.ownedTables:["table_classic"];S.equippedTable=x.equippedTable||"table_classic";S.ownedDice=Array.isArray(x.ownedDice)?x.ownedDice:["dice_ivory"];S.equippedDice=x.equippedDice||"dice_ivory";S.achievements=Array.isArray(x.achievements)?x.achievements:[]}catch{}}
 ...
 async function persistYandex(){if(!S.player)return;try{await S.player.setStats({rating:S.rating,pointsBalance:S.balance,wins:S.wins,games:S.games})}catch{}try{await S.player.setData({partisName:S.name,ownedSkins:S.owned,equippedSkin:S.equipped,partisAvatar:S.avatar,partisOwnedTables:S.ownedTables,partisTable:S.equippedTable,partisOwnedDice:S.ownedDice,partisDice:S.equippedDice,partisAchievements:S.achievements,partisWinStreak:S.winStreak,partisBestWinStreak:S.bestWinStreak},true)}catch{}if(S.authorized&&window.ysdk?.leaderboards){clearTimeout(leaderboardDebounce);leaderboardDebounce=setTimeout(async()=>{try{const ok=await window.ysdk.isAvailableMethod?.("leaderboards.setScore");if(ok!==false)await window.ysdk.leaderboards.setScore("rating",S.rating)}catch{}},1200)}}
 async function initPlayer(){loadFallback();S.id=fallbackId();const hadLocalName=S.name&&S.name!=="Игрок";const ysdk=await waitSdk();if(ysdk){try{const p=await ysdk.getPlayer();S.player=p;S.id=cleanId(p.getUniqueID?.()||S.id)||S.id;S.authorized=!!p.isAuthorized?.();const nm=String(p.getName?.()||"").trim();if(!hadLocalName&&S.authorized&&nm&&nm!=="unauthorized")S.name=nm.slice(0,24);try{const stats=await p.getStats(["rating","pointsBalance","wins","games"]);S.rating=Math.max(S.rating,+stats.rating||0);S.balance=Math.max(S.balance,+stats.pointsBalance||0);S.wins=Math.max(S.wins,+stats.wins||0);S.games=Math.max(S.games,+stats.games||0)}catch{}try{const data=await p.getData(["partisName","ownedSkins","equippedSkin","partisAvatar","partisOwnedTables","partisTable","partisOwnedDice","partisDice","partisAchievements","partisWinStreak","partisBestWinStreak"]);if(data.partisName)S.name=String(data.partisName).trim().slice(0,24)||S.name;if(Array.isArray(data.ownedSkins))S.owned=[...new Set(["default",...data.ownedSkins])];if(data.equippedSkin)S.equipped=data.equippedSkin;if(data.partisAvatar)S.avatar=data.partisAvatar;if(Array.isArray(data.partisOwnedTables))S.ownedTables=[...new Set(["table_classic",...data.partisOwnedTables])];if(data.partisTable)S.equippedTable=data.partisTable;if(Array.isArray(data.partisOwnedDice))S.ownedDice=[...new Set(["dice_ivory",...data.partisOwnedDice])];if(data.partisDice)S.equippedDice=data.partisDice;if(Array.isArray(data.partisAchievements))S.achievements=[...new Set(data.partisAchievements)];if(data.partisWinStreak!=null)S.winStreak=Math.max(S.winStreak,+data.partisWinStreak||0);if(data.partisBestWinStreak!=null)S.bestWinStreak=Math.max(S.bestWinStreak,+data.partisBestWinStreak||0)}catch{}}catch(e){console.warn("Yandex player unavailable",e)}}await syncServer();await claimStoredAchievementRewards();renderProfile();renderShop();renderProfileModal();renderDaily();loadDaily();loadLeaderboard();await loadSocial();startInvitePolling();initPayments();applyCosmetics();checkProfileAchievements()}
 ...
 async function loadLeaderboard(){const box=document.querySelector("#ratingList");if(box){box.innerHTML="";const l=document.createElement("div");l.className="ratingLoading";l.textContent=uiText("Загрузка рейтинга…");box.appendChild(l)};try{const d=await api("/api/leaderboard?limit=30");const rows=d.entries||[];if(box){box.innerHTML="";rows.forEach((x,i)=>{const row=document.createElement("div");row.className="ratingRow"+(x.id===S.id?" me":"");const av=document.createElement("div");av.className="ratingAvatar";paintAvatar(av,x.avatar);const place=document.createElement("span");place.className="ratingPlace";place.textContent=i+1;const name=document.createElement("span");name.className="ratingName";name.textContent=x.name||"Игрок";const score=document.createElement("b");score.textContent=x.rating||0;row.append(place,av,name,score);box.appendChild(row)});if(!rows.length)box.innerHTML='<div class="ratingLoading">Рейтинг пока пуст. Заведите первую фишку в Домик!</div>'}}catch(e){if(box){box.innerHTML="";const l=document.createElement("div");l.className="ratingLoading";l.textContent=uiText("Не удалось загрузить рейтинг.");box.appendChild(l)}}}
 function skinCard(s){const owned=S.owned.includes(s.id),equipped=S.equipped===s.id;const d=document.createElement("div");d.className="skinCard"+(equipped?" equipped":"");const piece=document.createElement("div");piece.className="skinPiece";piece.style.background=s.color||"linear-gradient(145deg,#ff6873,#c7152b)";d.appendChild(piece);const name=document.createElement("b");name.textContent=uiText(s.name);d.appendChild(name);const cost=document.createElement("small");cost.textContent=uiText(s.free?"Базовая":"200 очков");d.appendChild(cost);const btn=document.createElement("button");btn.type="button";if(equipped){btn.textContent=uiText("Выбрано");btn.disabled=true}else if(owned){btn.textContent=uiText("Выбрать");btn.onclick=()=>equipSkin(s.id)}else{btn.textContent=uiText("Купить за 200");btn.disabled=S.balance<200;btn.onclick=()=>buySkin(s.id)}d.appendChild(btn);return d}
 function countrySkinCard(s){const owned=S.owned.includes(s.skinId),equipped=S.equipped===s.skinId;const d=document.createElement("div");d.className="skinCard countrySkinCard premium"+(equipped?" equipped":"");const piece=document.createElement("div");piece.className="skinPiece countryPiece";piece.style.background=s.preview;const badge=document.createElement("span");badge.className="countryEmoji";badge.textContent=s.emoji;piece.appendChild(badge);d.appendChild(piece);const name=document.createElement("b");name.textContent=uiText(s.name);d.appendChild(name);const prod=S.catalog.get(s.productId);const cost=document.createElement("small");cost.textContent=prod?.price||uiText("Премиум");d.appendChild(cost);const btn=document.createElement("button");btn.type="button";if(equipped){btn.textContent=uiText("Выбрано");btn.disabled=true}else if(owned){btn.textContent=uiText("Выбрать");btn.onclick=()=>equipSkin(s.skinId)}else if(!prod){btn.textContent=uiText("Скоро");btn.disabled=true}else{btn.textContent=uiText("Купить");btn.onclick=()=>buyPremium(s)}d.appendChild(btn);return d}
 function cosmeticCard(item,kind){const owned=(kind==="table"?S.ownedTables:S.ownedDice).includes(item.id);const equipped=(kind==="table"?S.equippedTable:S.equippedDice)===item.id;const d=document.createElement("div");d.className="skinCard cosmeticCard"+(equipped?" equipped":"");const preview=document.createElement("div");preview.className=kind==="table"?"tablePreview":"dicePreview";preview.style.background=item.preview;if(kind==="dice")preview.textContent="⚄";d.appendChild(preview);const b=document.createElement("b");b.textContent=uiText(item.name);d.appendChild(b);const sm=document.createElement("small");sm.textContent=uiText(item.price?`${item.price} очков`:"Базовый");d.appendChild(sm);const btn=document.createElement("button");btn.type="button";if(equipped){btn.textContent=uiText("Выбрано");btn.disabled=true}else if(owned){btn.textContent=uiText("Выбрать");btn.onclick=()=>equipCosmetic(kind,item.id)}else{btn.textContent=uiText(`Купить за ${item.price}`);btn.disabled=S.balance<item.price;btn.onclick=()=>buyCosmetic(kind,item.id)}d.appendChild(btn);return d}
 function renderShop(){const grid=document.querySelector("#skinGrid");if(grid){grid.innerHTML="";SKINS.forEach(s=>grid.appendChild(skinCard(s)))}const cg=document.querySelector("#countryGrid");if(cg){cg.innerHTML="";COUNTRY_SKINS.forEach(s=>cg.appendChild(countrySkinCard(s)))}const tg=document.querySelector("#tableGrid");if(tg){tg.innerHTML="";TABLES.forEach(s=>tg.appendChild(cosmeticCard(s,"table")))}const dg=document.querySelector("#diceGrid");if(dg){dg.innerHTML="";DICE.forEach(s=>dg.appendChild(cosmeticCard(s,"dice")))}const prem=document.querySelector("#premiumGrid");if(prem){prem.innerHTML="";CLUB_PRODUCTS.forEach(c=>{const owned=S.owned.includes(c.skinId),equipped=S.equipped===c.skinId,v=CLUB_VISUALS[c.skinId];const d=document.createElement("div");d.className="skinCard premium"+(equipped?" equipped":"");const piece=document.createElement("div");piece.className="skinPiece clubPiece";piece.style.background=`linear-gradient(90deg,${v.color} 0 48%,${v.stripe} 48% 58%,${v.color} 58%)`;d.appendChild(piece);const name=document.createElement("b");name.textContent=uiText(c.name);d.appendChild(name);const price=document.createElement("small");const prod=S.catalog.get(c.productId);price.textContent=prod?.price||uiText("Премиум");d.appendChild(price);const btn=document.createElement("button");btn.type="button";if(equipped){btn.textContent=uiText("Выбрано");btn.disabled=true}else if(owned){btn.textContent=uiText("Выбрать");btn.onclick=()=>equipSkin(c.skinId)}else if(!window.MONDAVOSHKA_CLUB_LICENSED||!prod){btn.textContent=uiText("Скоро");btn.disabled=true}else{btn.textContent=uiText("Купить");btn.onclick=()=>buyPremium(c)}d.appendChild(btn);prem.appendChild(d)})}}
 ...
 function avatarChoiceButton(a){const b=document.createElement("button");b.type="button";b.className="avatarChoice"+(S.avatar===a.id?" selected":"");b.title=uiText(a.name);b.innerHTML=`<span>${a.emoji}</span><small>${uiText(a.name)}</small>`;b.onclick=()=>setAvatar(a.id);return b}
 async function saveProfileName(){
   const input=document.querySelector("#profileNameInput");
   const btn=document.querySelector("#saveProfileName");
   const hint=document.querySelector("#profileNameHint");
   let name=String(input?.value||"").replace(/[<>\u0000-\u001f]/g,"").trim().replace(/\s+/g," ");
   if(name.length<2){
     if(hint)hint.textContent=uiText("Имя должно содержать минимум 2 символа.");
     input?.focus();return;
   }
   if(name.length>24)name=name.slice(0,24).trim();
   if(btn){btn.disabled=true;btn.textContent=uiText("Сохранение…")}
   try{
     const d=await api("/api/update-name",{method:"POST",body:JSON.stringify({id:S.id,name})});
     if(d.profile)applyServerProfile(d.profile,false);
     else S.name=name;
     saveFallback();renderProfile();renderProfileModal();persistYandex();
     if(hint)hint.textContent=uiText("Имя сохранено. Оно будет использоваться во всех новых партиях.");
   }catch(e){
     if(hint)hint.textContent=e.message||uiText("Не удалось сохранить имя.");
   }finally{
     if(btn){btn.disabled=false;btn.textContent=uiText("Сохранить")}
   }
 }
 ...
 async function claimStoredAchievementRewards(){
  ...
  try{
   ...
   if(d?.profile)applyServerProfile(d.profile,false);
   if((Number(d?.reward)||0)>0){const toast=document.querySelector("#ratingToast");if(toast){toast.textContent=uiText(`🏅 +${d.reward} очков за ранее полученные достижения`);toast.classList.add("show");setTimeout(()=>toast.classList.remove("show"),3000)}}
 ...
 function checkProfileAchievements(){
  ...
  if(S.winStreak>=3)unlockAchievement("streak3");
  if(S.winStreak>=5)unlockAchievement("streak5");
  if(S.winStreak>=10)unlockAchievement("streak10");
 }
 async function unlockAchievement(id){
  ...
  try{
   ...
   if(toast){toast.textContent=uiText(`🏅 ${a.name}${reward?` · +${reward} очков`:""}`);toast.classList.add("show");setTimeout(()=>toast.classList.remove("show"),2800)}
   return true;
  }catch(e){
 ...
 function dailyQuestName(q){return window.PartisI18n?.dailyName?.(q.id,q.name)||q.name}
 function renderDaily(){
   const grid=document.querySelector("#dailyQuestGrid"),summary=document.querySelector("#dailyMenuSummary");
   const quests=S.daily?.quests||[];
   if(summary){
     const done=quests.filter(q=>q.claimed).length;
     summary.textContent=uiText(quests.length?`${done}/${quests.length} выполнено · награды на баланс`:"3 задания · награды на баланс");
   }
   if(!grid)return;
   grid.replaceChildren();
   if(!quests.length){const l=document.createElement("div");l.className="ratingLoading";l.textContent=uiText("Загрузка заданий…");grid.appendChild(l);return}
   for(const q of quests){
     const d=document.createElement("div");d.className="dailyQuestCard"+(q.claimed?" done":"");
     const top=document.createElement("div");top.className="dailyQuestTop";
     const name=document.createElement("b");name.textContent=(q.claimed?"✅ ":"")+dailyQuestName(q);
     const reward=document.createElement("strong");reward.textContent=`+${q.reward} 💰`;
     top.append(name,reward);
     const progress=document.createElement("div");progress.className="dailyQuestProgress";
     const bar=document.createElement("i");bar.style.width=`${Math.max(0,Math.min(100,(Number(q.progress)||0)/(Number(q.target)||1)*100))}%`;
     progress.appendChild(bar);
     const text=document.createElement("small");text.textContent=uiText(`${Math.min(q.target,Number(q.progress)||0)} / ${q.target}${q.claimed?" · выполнено":""}`);
     d.append(top,progress,text);grid.appendChild(d);
   }
 }
 async function loadDaily(){
   if(!S.id)return;
   try{const d=await api(`/api/daily?id=${encodeURIComponent(S.id)}`);if(d?.profile)applyServerProfile(d.profile,false);else if(d?.daily)S.daily=d.daily;renderDaily()}
   catch(e){console.warn("Daily load failed",e)}
 }
 async function reportDaily(event,amount=1){
   if(!S.id)return null;
   try{
     const d=await api("/api/daily-progress",{method:"POST",body:JSON.stringify({id:S.id,event,amount})});
     if(d?.profile)applyServerProfile(d.profile,false);else if(d?.daily)S.daily=d.daily;
     renderDaily();
     if((Number(d?.reward)||0)>0){
       const toast=document.querySelector("#ratingToast");
       if(toast){toast.textContent=uiText(`📅 Ежедневное задание · +${d.reward} очков`);toast.classList.add("show");setTimeout(()=>toast.classList.remove("show"),2800)}
     }
     return d;
   }catch(e){console.warn("Daily progress failed",e);return null}
 }
 async function reportLocalMatch(matchId,won){
   if(!S.id||!matchId)return null;
   try{
     const d=await api("/api/local-match-result",{method:"POST",body:JSON.stringify({id:S.id,matchId,won:!!won})});
     if(d?.profile)applyServerProfile(d.profile,false);
     renderDaily();checkProfileAchievements();
     if((Number(d?.reward)||0)>0){
       const toast=document.querySelector("#ratingToast");
       if(toast){toast.textContent=uiText(`📅 Задание выполнено · +${d.reward} очков`);toast.classList.add("show");setTimeout(()=>toast.classList.remove("show"),2800)}
     }
     return d;
   }catch(e){console.warn("Local match result failed",e);return null}
 }
 function dailyAwardFromServer(profile,reward=0){
   if(profile)applyServerProfile(profile,false);
   renderDaily();checkProfileAchievements();
   if(Number(reward)>0){
     const toast=document.querySelector("#ratingToast");
 ...
 document.querySelector("#saveProfileName")?.addEventListener("click",saveProfileName);
 document.querySelector("#profileNameInput")?.addEventListener("keydown",e=>{if(e.key==="Enter"){e.preventDefault();saveProfileName()}});
